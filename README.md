# 👋 Hi, I'm Mohammad Riaz

🎓 BCA (AI & ML) Student — Manav Rachna International Institute of Research & Studies
💻 Aspiring Frontend Web Developer
📍 Faridabad, Haryana, India

---

## 🚀 About Me

* Passionate about building responsive and user-friendly web applications
* Currently learning **React.js** and improving problem-solving skills
* Interested in Frontend Development & UI/UX Design
* Looking for **Internship / Entry-Level Developer opportunities**

---

## 🛠️ Tech Stack

**Frontend**

* HTML
* CSS
* JavaScript
* React.js

**Programming Languages**

* Python
* C
* Java

**Tools**

* Git & GitHub
* VS Code
* Oracle Database (Basics)

---

## 📂 Projects

### 🔹 Online Quiz Application

Interactive quiz web app that loads questions dynamically and calculates score automatically.
Tech Used: HTML, CSS, JavaScript
🔗 https://github.com/Riaz1909/quiz_app

### 🔹 Real-Time Chat App (React)

Responsive chat interface using React components and UI state management.
🔗 https://github.com/Riaz1909/react-responsive-chat

### 🔹 Portfolio Website

Personal responsive portfolio built using HTML & CSS.
🔗 https://github.com/Riaz1909/Portfolio

### 🔹 E-Learning Platform UI

Multi-page e-learning platform UI focused on navigation and responsive design.
🔗 https://github.com/Riaz1909/E-learning-platform

---

## 📫 Contact Me

📧 Email: [riazmohd98769@gmail.com](mailto:riazmohd98769@gmail.com)
📱 Phone: +91-9968443388

---

⭐ *Always learning. Always building.*
